...
SerialParser::SerialParser(PinName tx, PinName rx , MotorCtrl& _asser):SerialCom(tx, rx), asser(_asser)
{
    initCst();   // inititate mtotor Ctrl Constants to define values in the .h, waiting for further notice from the COM
    
    com_timeout.start(); //start the timeout timer
}
...
void SerialParser::printDebug(char* message)
{
...
bool SerialParser::checkComTimeout(){
    
    if (com_timeout.read() > COM_TIMEOUT )
        {
            resetComData();
            static int countDebugTO = 0;
            countDebugTO++;
            if (countDebugTO > 10)
            {
                printf("DeCom_Timeout\n");
                countDebugTO = 0;
            }  
...
bool SerialParser::interpretData(){
    ...
    case 'T':   Ttwist = incom_data; debug_print("Ttwist %f\n", Ttwist);  com_timeout.reset();  break; //asser.angleCommand.SetSpeed(Ttwist);
    case 'V':   Vtwist = incom_data; debug_print("Vtwist %f\n", Vtwist);  com_timeout.reset();  break; //asser.distanceCommand.SetSpeed(Vtwist);
    
    case 'S':   SStatus = (int)incom_data;  //Reset
                asser.resetServoingParam();
                break;
                
    case 'U':   UPower = (bool)incom_data; debug_print("DeUPower %d\n", UPower); asser.enable(UPower); resetComData(); break;
